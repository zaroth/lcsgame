...
void noticecheck(int exclude) {
    ...
    if(topi >= 0 && noticer.size())
        activesquad->squad[topi]->train(SKILL_THEFT, 10);
...
char alienationcheck(char mistake) {
    ...
    for(int e = 0; e < ENCMAX; e++) {
        ...
        if(!strcmp(encounter[e].name, "Prisoner"))
            continue;
    ...
    if(location[cursite]->siege.siege) {
        ...
        case SITE_GOVERNMENT_POLICESTATION:
            if(levelmap[locx][locy][locz].flag & SITEBLOCK_RESTRICTED) {
                ...
                if(cr.armor.type == ARMOR_POLICEARMOR)
                    uniformed = 1;
            }
        ...
        case SITE_GOVERNMENT_COURTHOUSE:
            if(levelmap[locx][locy][locz].flag & SITEBLOCK_RESTRICTED) {
                ...
                if(cr.armor.type == ARMOR_BLACKDRESS)
                    uniformed = 1;
