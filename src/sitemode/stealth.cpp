...
void noticecheck(int32 exclude) {
    ...
    for(int32 e = 0; e < ENCMAX; e++) {
        ...
        if(encounter[e].type == CREATURE_PRISONER)
            continue;
...
char alienationcheck(char mistake) {
    ...
    for(int32 e = 0; e < ENCMAX; e++) {
        ...
        if(encounter[e].alive && encounter[e].exists &&
            ...
            if(encounter[e].cantbluff == 0)
                encounter[e].cantbluff = 1;
...
void disguisecheck(void) {
    ...
    if(sitealarmtimer == -1 && weapon < 2) {
        if(!disguisesite(location[cursite]->type) &&
                !(map[locx][locy][locz].flag & SITEBLOCK_RESTRICTED))
            return;
    }
    ...
    for(int32 p = 0; p < 6; p++) {
        if(activesquad->squad[p] != NULL) {
            ...
            if(activesquad->squad[p]->armor.quality != '1') {
                if(activesquad->squad[p]->armor.quality == '2')
                    skill--;
                else if(activesquad->squad[p]->armor.quality == '3')
                    skill >>= 1;
                else if(activesquad->squad[p]->armor.quality == '4')
                    skill >>= 2;
            }
...
char weaponcheck(creaturest &cr, int16 type) {
    ...
    case WEAPON_CARBINE_M4:
        if(cr.armor.type == ARMOR_TRENCHCOAT)
            suspicious = 0;

        if(cr.armor.type == ARMOR_POLICEUNIFORM && hasdisguise(cr, type))
            suspicious = 0;

        if(cr.armor.type == ARMOR_BALLISTICVEST && cr.armor.subtype == BVEST_POLICE && hasdisguise(cr, type))
            suspicious = 0;

        if(law[LAW_GUNCONTROL] == -2 && cr.armor.type == ARMOR_SECURITYUNIFORM && hasdisguise(cr, type))
            suspicious = 0;

    case WEAPON_AUTORIFLE_M16:
    case WEAPON_SEMIRIFLE_AR15:
        if(law[LAW_POLICEBEHAVIOR] == -2 && law[LAW_DEATHPENALTY] == -2 &&
                cr.armor.type == ARMOR_POLICEUNIFORM && hasdisguise(cr, type))
            suspicious = 0;

        if(law[LAW_POLICEBEHAVIOR] == -2 && law[LAW_DEATHPENALTY] == -2 &&
                cr.armor.type == ARMOR_BALLISTICVEST && cr.armor.subtype == BVEST_POLICE && hasdisguise(cr, type))
            suspicious = 0;

        if(cr.armor.type == ARMOR_MILITARY && hasdisguise(cr, type))
            suspicious = 0;

        if(cr.armor.type == ARMOR_BALLISTICVEST && cr.armor.subtype == BVEST_MILITARY && hasdisguise(cr, type))
            suspicious = 0;
...
char hasdisguise(creaturest &cr, int16 type) {
    ...
    if(location[cursite]->siege.siege) {
        ...
        case SIEGE_CIA: {
            if(cr.armor.type == ARMOR_BLACKSUIT)
                uniformed = 1;
        ...
        case SIEGE_HICKS: {
            if(cr.armor.type == ARMOR_CLOTHES)
                uniformed = 1;

            if(cr.armor.type == ARMOR_OVERALLS)
                uniformed = 1;

            if(cr.armor.type == ARMOR_WIFEBEATER)
                uniformed = 1;
        ...
        case SIEGE_POLICE: {
            ...
            if(cr.armor.type == ARMOR_BALLISTICVEST && cr.armor.subtype == BVEST_POLICE)
                uniformed = 1;

            if(cr.armor.type == ARMOR_MILITARY &&
                    location[cursite]->siege.escalationstate > 0)
                uniformed = 1;
        ...
        case SITE_LABORATORY_GENETIC:
            if(map[locx][locy][locz].flag & SITEBLOCK_RESTRICTED) {
                if(cr.armor.type == ARMOR_LABCOAT)
                    uniformed = 1;

                if(cr.armor.type == ARMOR_SECURITYUNIFORM)
                    uniformed = 1;
            } else if(cr.armor.type != ARMOR_NONE && cr.armor.type != ARMOR_HEAVYBALLISTICVEST)
                uniformed = 1;
        ...
        case SITE_GOVERNMENT_COURTHOUSE:
            if(map[locx][locy][locz].flag & SITEBLOCK_RESTRICTED) {
                if(cr.armor.type == ARMOR_BLACKROBE)
                    uniformed = 1;
        ...
        case SITE_GOVERNMENT_PRISON:
            if(map[locx][locy][locz].flag & SITEBLOCK_RESTRICTED) {
                if(law[LAW_DEATHPENALTY] == -2 &&
                    ...
                    if(cr.armor.type == ARMOR_LABCOAT)
                        uniformed = 1;
                } else if(cr.armor.type == ARMOR_PRISONGUARD)
                    uniformed = 1;

                if(cr.armor.type == ARMOR_PRISONER)
                    uniformed = 1;
            } else if(cr.armor.type != ARMOR_NONE && cr.armor.type != ARMOR_HEAVYBALLISTICVEST)
                uniformed = 1;
        ...
        case SITE_BUSINESS_CIGARBAR:
            ...
            if(cr.armor.type == ARMOR_EXPENSIVESUIT)
                uniformed = 1;

            if(cr.armor.type == ARMOR_CHEAPSUIT)
                uniformed = 1;

            if(cr.armor.type == ARMOR_EXPENSIVEDRESS)
                uniformed = 1;

            if(cr.armor.type == ARMOR_CHEAPDRESS)
                uniformed = 1;
        ...
        case SITE_INDUSTRY_POLLUTER:
            if(cr.armor.type == ARMOR_WORKCLOTHES)
                uniformed = 1;
        ...
        default:
            if(cr.armor.type != ARMOR_NONE && cr.armor.type != ARMOR_HEAVYBALLISTICVEST)
                uniformed = 1;
