...
void creatureadvance(void) {
            ...
            if(!activesquad->squad[p]->prisoner->alive) {
                if(activesquad->squad[p]->prisoner->squadid == -1) {
                    ...
                    if(activesquad->squad[p]->prisoner->type == CREATURE_CORPORATE_CEO ||
                            ...
                            activesquad->squad[p]->prisoner->type == CREATURE_JUDGE_CONSERVATIVE)
                        sitecrime += 30;
    ...
    if (gamelog.logged_since_last_message)
        gamelog.nextMessage();
}
    ...
    if(mode == GAMEMODE_SITE && LCSrandom(3) &&
        ...
        if(cr.blood <= 0) {
            ...
            if(cr.squadid != -1) {
                ...
                if(location[cursite]->siege.siege && cr.animalgloss == ANIMALGLOSS_TANK)
                    location[cursite]->siege.tanks--;

                if(location[cursite]->renting == RENTING_CCS) {
                    if(cr.type == CREATURE_CCS_ARCHCONSERVATIVE)
                        ccs_boss_kills++;
