        ...
        for(int32 y = 0; y < MAPY; y++) {
            for(int32 z = 0; z < MAPZ; z++) {
                if(map[x][y][z].flag & SITEBLOCK_DOOR) {
                    ...
                    if(x > 0)if(!(map[x - 1][y][z].flag & SITEBLOCK_BLOCK)) {
                            block &= ~BIT2;
                            opennum++;
                        }

                    if(x < MAPX - 1)if(!(map[x + 1][y][z].flag & SITEBLOCK_BLOCK)) {
                            block &= ~BIT3;
                            opennum++;
                        }

                    if(y > 0)if(!(map[x][y - 1][z].flag & SITEBLOCK_BLOCK)) {
                            block &= ~BIT1;
                            opennum++;
                        }

                    if(y < MAPY - 1)if(!(map[x][y + 1][z].flag & SITEBLOCK_BLOCK)) {
                            block &= ~BIT4;
                            opennum++;
                        }
                    ...
                    if(block == (BIT1 | BIT2 | BIT3 | BIT4)) {
                        if(x > 0)
                            map[x - 1][y][z].flag &= ~SITEBLOCK_BLOCK;

                        if(x < MAPX - 1)
                            map[x + 1][y][z].flag &= ~SITEBLOCK_BLOCK;

                        if(y > 0)
                            map[x][y - 1][z].flag &= ~SITEBLOCK_BLOCK;
...
void generateroom(int32 rx, int32 ry, int32 dx, int32 dy, int32 z) {
    ...
    if((!LCSrandom(2) || dy <= 2) && dx > 2) {
        ...
        for(int32 wy = 0; wy < dy; wy++)
            map[wx][ry + wy][z].flag = SITEBLOCK_BLOCK;

        int32 rny = LCSrandom(dy);
        map[wx][ry + rny][z].flag = SITEBLOCK_DOOR;

        if(!LCSrandom(3))
            map[wx][ry + rny][z].flag |= SITEBLOCK_LOCKED;
        ...
        for(int32 wx = 0; wx < dx; wx++)
            map[rx + wx][wy][z].flag = SITEBLOCK_BLOCK;

        int32 rnx = LCSrandom(dx);
        map[rx + rnx][wy][z].flag = SITEBLOCK_DOOR;

        if(!LCSrandom(3))
            map[rx + rnx][wy][z].flag |= SITEBLOCK_LOCKED;
...
void knowmap(int32 locx, int32 locy, int32 locz) {
    ...
    if(locx > 0)
        map[locx - 1][locy][locz].flag |= SITEBLOCK_KNOWN;

    if(locx < MAPX - 1)
        map[locx + 1][locy][locz].flag |= SITEBLOCK_KNOWN;

    if(locy > 0)
        map[locx][locy - 1][locz].flag |= SITEBLOCK_KNOWN;
