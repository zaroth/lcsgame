...
void printwall(int x, int y, int z, int px, int py) {
    ...
    if(y < locy && y > 0)
        visible[WALL_DOWN] = true;
    else if(y > locy && y < MAPY)
        visible[WALL_UP] = true;

    if(x < locx && x > 0)
        visible[WALL_RIGHT] = true;
    else if(x > locx && x < MAPX)
        visible[WALL_LEFT] = true;

    // 2) Check LOS
    if(!LineOfSight(x - 1, y, z))
        visible[WALL_LEFT] = false;

    if(!LineOfSight(x + 1, y, z))
        visible[WALL_RIGHT] = false;

    if(!LineOfSight(x, y - 1, z))
        visible[WALL_UP] = false;

    if(!LineOfSight(x, y + 1, z))
        visible[WALL_DOWN] = false;

    // 3) Check for memory
    if(levelmap[x - 1][y][z].flag & (SITEBLOCK_KNOWN))
        visible[WALL_LEFT] = true;

    if(levelmap[x + 1][y][z].flag & (SITEBLOCK_KNOWN))
        visible[WALL_RIGHT] = true;

    if(levelmap[x][y - 1][z].flag & (SITEBLOCK_KNOWN))
        visible[WALL_UP] = true;
    ...
    if(levelmap[x - 1][y][z].flag & (SITEBLOCK_BLOCK | SITEBLOCK_DOOR))
        visible[WALL_LEFT] = false;

    if(levelmap[x + 1][y][z].flag & (SITEBLOCK_BLOCK | SITEBLOCK_DOOR))
        visible[WALL_RIGHT] = false;

    if(levelmap[x][y - 1][z].flag & (SITEBLOCK_BLOCK | SITEBLOCK_DOOR))
        visible[WALL_UP] = false;
    ...
    if(levelmap[x][y][z].flag & SITEBLOCK_BLOCK) {
        ...
        if(levelmap[x - 1][y][z].flag & SITEBLOCK_BLOODY2)
            bloody[WALL_LEFT] = true;

        if(levelmap[x + 1][y][z].flag & SITEBLOCK_BLOODY2)
            bloody[WALL_RIGHT] = true;
