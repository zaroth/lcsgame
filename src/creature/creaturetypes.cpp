...
void makecreature(Creature &cr, short type) {
    ...
    case CREATURE_CORPORATE_CEO:
        ...
        if(LCSrandom(10))
            cr.gender_conservative = GENDER_MALE;
        else if(cr.gender_conservative == GENDER_FEMALE)
            cr.set_skill(SKILL_DISGUISE, LCSrandom(3) + 4);
    ...
    case CREATURE_WORKER_SERVANT:
        cr.money = 0;
        cr.align = 1;
        cr.armor.type = ARMOR_SERVANTUNIFORM;
        cr.age = AGE_MATURE;
        break;
    ...
    case CREATURE_GENETIC:
        ...
        case 6:
            strcat(cr.name, "Purple Gorilla");
            break;

        case 7:
            strcat(cr.name, "Warped Bear");
            break;

        case 8:
            strcat(cr.name, "Writhing Mass");
            break;
    ...
    case CREATURE_BUM:
        ...
        if(cr.weapon.type == WEAPON_NONE && !LCSrandom(5))
    ...
    if(cr.type == CREATURE_MUTANT)
        attnum = LCSrandom(60) + 7;
    ...
    while(randomskills > 0) {
        ...
        if(LCSrandom(20)) {
            if(randomskill == SKILL_FLAMETHROWER)
                continue;

            if(randomskill == SKILL_SMG)
                continue;

            if(randomskill == SKILL_SWORD)
                continue;

            if(randomskill == SKILL_RIFLE)
                continue;

            if(randomskill == SKILL_AXE)
                continue;

            if(randomskill == SKILL_CLUB)
                continue;

            if(randomskill == SKILL_PSYCHOLOGY)
                continue;
        }
        ...
        if(LCSrandom(10) && cr.align != ALIGN_CONSERVATIVE) {
            if(randomskill == SKILL_SHOTGUN)
                continue;

            if(randomskill == SKILL_PISTOL)
                continue;
        }
