...
void Creature::set_skill(int skill, int amount) {
    skills[skill].value = amount;
}
int Creature::get_skill(int skill) const {
...
int Creature::get_attribute(int attribute, bool usejuice) const {
    ...
    if(attribute == ATTRIBUTE_WISDOM && align != ALIGN_CONSERVATIVE)
        usejuice = false;

    if(attribute == ATTRIBUTE_HEART  && align != ALIGN_LIBERAL)
        usejuice = false;
    ...
    if(usejuice) {
        if(juice <= -50)
            ...
            if(juice < 50)
                ret = static_cast<int>(ret += 1);  // Activist
            else if(juice < 100)
...
char Creature::gender() const {
    char gender = (law[LAW_GAY] == -2 ? gender_conservative :
                   gender_liberal); // use biological gender instead of gender label if Gay Rights is C+

    if(gender == GENDER_WHITEMALEPATRIARCH)
        gender = GENDER_MALE;  // functions calling gender() probably don't expect to see GENDER_WHITEMALEPATRIARCH
...
const char *Creature::heshe() const {
    ...
    default:
        if(law[LAW_GAY] == -2)
            return "it";  // intersex people aren't even considered people if Gay Rights is C+
...
const char *Creature::hisher() const {
    ...
    default:
        if(law[LAW_GAY] == -2)
            return "its";  // intersex people aren't even considered people if Gay Rights is C+
