...
void review(void) {
    ...
    do {
        ...
        if((c == interface_pgdn || c == KEY_DOWN || c == KEY_RIGHT) && (page + 1) * 19 < (int)squad.size() + REVIEWMODENUM)
            page++;
...
void squadlessbaseassign(void) {
    ...
    for(p = 0; p < (int)pool.size(); p++)if(pool[p]->is_active_liberal() && pool[p]->squadid == -1)
            temppool.push_back(pool[p]);
    ...
    do {
        ...
        //PAGE DOWN
        if((c == interface_pgdn || c == KEY_DOWN || c == KEY_RIGHT) && (page_lib + 1) * 19 < (int)temppool.size())
            page_lib++;
...
#define PAGELENGTH 19
    ...
    do {
        ...
        //PAGE DOWN
        if((c == interface_pgdn || c == KEY_DOWN || c == KEY_RIGHT) && (page + 1)*PAGELENGTH < (int)temppool.size())
            page++;
