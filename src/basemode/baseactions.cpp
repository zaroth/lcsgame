...
void burnflag(void) {
    ...
    while(flagparts > 0) {
        if(!first) {
            for(x = 0; x < 16; x++) {
                for(y = 0; y < 7; y++) {
                    if(flag[x][y][0] == CH_BOX_DRAWINGS_LIGHT_VERTICAL)
                        flag[x][y][0] = CH_DARK_SHADE;
            ...
            if(flag[x][y][0] == ':' || flag[x][y][0] == CH_UPPER_HALF_BLOCK || flag[x][y][0] == CH_LOWER_HALF_BLOCK) {
                if(x > 0) {
                    if(flag[x - 1][y][0] != ':' &&
                            ...
                            flag[x - 1][y][0] != CH_LOWER_HALF_BLOCK)
                        conf = 1;
                }
                ...
                if(y > 0) {
                    if(flag[x][y - 1][0] != ':' &&
                            ...
                            flag[x][y - 1][0] != CH_LOWER_HALF_BLOCK)
                        conf = 1;
                }
...
void stopevil(void) {
    ...
    do {
        ...
        if((loc == -1) || (multipleCityMode && location[loc]->type == squad_location->city))
            addstr("Enter - The squad is not yet Liberal enough.");
        else
            addstr("Enter - Back one step.");
