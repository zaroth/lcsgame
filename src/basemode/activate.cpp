...
void activate(void) {
    ...
    do {
        ...
        //PAGE DOWN
        if((c == interface_pgdn || c == KEY_DOWN || c == KEY_RIGHT) && (page + 1) * 19 < (int)temppool.size())
            page++;
...
void updateclasschoice(Creature *cr, char choice) {
    if (choice == '6') {
        ...
        if (classlist == 0) {
            switch(choice) {
            case '1':
                cr->activity.type = ACTIVITY_STUDY_DEBATING;
                break;

            case '2':
                cr->activity.type = ACTIVITY_STUDY_BUSINESS;
                break;

            case '3':
                cr->activity.type = ACTIVITY_STUDY_PSYCHOLOGY;
                break;

            case '4':
                cr->activity.type = ACTIVITY_STUDY_LAW;
                break;

            case '5':
                cr->activity.type = ACTIVITY_STUDY_SCIENCE;
                break;
            }
        } else if (classlist == 1) {
            switch(choice) {
            case '1':
                cr->activity.type = ACTIVITY_STUDY_DRIVING;
                break;

            case '2':
                cr->activity.type = ACTIVITY_STUDY_FIRST_AID;
                break;
            ...
            case '4':
                cr->activity.type = ACTIVITY_STUDY_DISGUISE;
                break;

            case '5':
                cr->activity.type = ACTIVITY_STUDY_MARTIAL_ARTS;
                break;
            }
        } else if (classlist == 2) {
            switch(choice) {
            case '1':
                cr->activity.type = ACTIVITY_STUDY_GYMNASTICS;
                break;

            case '2':
                cr->activity.type = ACTIVITY_STUDY_WRITING;
                break;

            case '3':
                cr->activity.type = ACTIVITY_STUDY_TEACHING;
                break;
    ...
    do {
        ...
        case 'd':
            ...
            if(cr->canwalk()) {
                ...
                if(!(cr->flag & CREATUREFLAG_WHEELCHAIR))
                    set_color(COLOR_WHITE, COLOR_BLACK, cr->activity.type == ACTIVITY_WHEELCHAIR);
                else
                    set_color(COLOR_BLACK, COLOR_BLACK, 1);
        ...
        if((c >= 'a' && c <= 'z') || (c >= '1' && c <= '9')) {
            ...
            case 'd':
                ...
                default:
                case '1':
                    break;//cr->activity.type=ACTIVITY_RECRUITING;break;
...
void recruitSelect(Creature &cr) {
    ...
    do {
        ...
        //PAGE DOWN
        if((c == interface_pgdn || c == KEY_DOWN || c == KEY_RIGHT) && (page + 1) * 19 < options)
            page++;
...
void select_makeclothing(Creature *cr) {
    ...
    do {
        ...
        //PAGE DOWN
        if((c == interface_pgdn || c == KEY_DOWN || c == KEY_RIGHT) && (page + 1) * 19 < (int)armortype.size())
            page++;
