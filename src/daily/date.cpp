...
char completevacation(datest &d, int p, char &clearformess) {
    ...
    default:
    case DATERESULT_MEETTOMORROW:
        return 0;

    case DATERESULT_BREAKUP:
        return 1;

    case DATERESULT_JOINED:
        return 1;

    case DATERESULT_ARRESTED:
        return 1;
    }
}
...
char completedate(datest &d, int p, char &clearformess) {
    ...
    if(d.date.size() == 1) {
        if(pool[p]->clinic)
            addstr("a \"hot\" date with ", gamelog);
        else
            addstr("a hot date with ", gamelog);
    } else
    ...
    for(e = d.date.size() - 1; e >= 0; e--) {
        ...
        if(!pool[p]->clinic && pool[p]->blood == 100)
            set_color(COLOR_WHITE, COLOR_BLACK, 0);
        else
            set_color(COLOR_BLACK, COLOR_BLACK, 1);
        ...
        if(pool[p]->blood == 100)
            ...
            addstr("C - Spend a week on a cheap vacation (must be uninjured).");
