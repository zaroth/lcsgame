...
void siegecheck(char canseethings) {
    ...
    for(int32 l = 0; l < location.size(); l++) {
        ...
        if(location[l]->siege.timeuntillocated == -2) {
            ...
            if(!crimes && location[l]->heat) {
                ...
                if(location[l]->siege.timeuntillocated == -1 &&
                    ...
                    int32 siegetime = 5 * (1 + 1 * heatprotection);
            ...
            if(!location[l]->siege.timeuntillocated) {
                ...
                if(numpres > 0) {
                    ...
                    for(int32 v = (int32)vehicle.size() - 1; v >= 0; v--) {
        ...
        //CIA
        if(location[l]->heat && location[l]->siege.timeuntilcia == -1 && !location[l]->siege.siege && offended_cia && !LCSrandom(600) &&
                numpres > 0) {
            location[l]->siege.timeuntilcia = LCSrandom(3) + 1;
            // *JDS* agent sleepers may give a warning before corp raids
            int32 agentsleepercount = 0;
            ...
            if(attack) {
                ...
                if(location[l]->siege.escalationstate >= 3 && !LCSrandom(20)) {
                    ...
                    if(pol.size() > 0) {
                        ...
                        set_color(COLOR_WHITE, COLOR_BLACK, 1);
                        move(8, 1);
                        int32 targ = pol[LCSrandom(pol.size())];
                ...
                if(!LCSrandom(50) && no_bad && livingpool) {
                    ...
                    int32 segmentpower = LCSrandom(bestvalue * 2 + 1);
                    ...
                    if(segmentpower < 10) {
                        ...
                        addstr("But the interview sucked.");
                    else if(segmentpower < 25)
                        addstr("It was nothing special, though.");
                    else if(segmentpower < 32)
                        addstr("It went pretty well.");
                    else if(segmentpower < 40)
                        addstr("The discussion was exciting and dynamic.");
                    else if(segmentpower < 50)
                        addstr("It was almost perfect.");
                    else {
                        addstr(repname);
...
void giveup(void) {
    ...
    if(location[loc]->siege.siegetype == SIEGE_POLICE) {
        ...
        for(p = pool.size() - 1; p >= 0; p--) {
            ...
            if(pool[p]->squadid != -1) {
                int32 sq = getsquad(pool[p]->squadid);
...
int32 fooddaysleft(int32 loc) {
    ...
    int32 days = location[loc]->compound_stores / eaters;
...
void escapesiege(char won) {
    ...
    if(!won) {
        ...
        for(int32 l2 = 0; l2 < location[cursite]->loot.size(); l2++)
...
void statebrokenlaws(int32 loc) {
    int16 breakercount[LAWFLAGNUM] = {0};
    int32 typenum = 0;
    int32 criminalcount = 0;
    char kname[100];
    int32 kidnapped = 0;
