...
void tendhostage(Creature *cr, char &clearformess) {
...

    {
        ...
        if(cr->joindays)
            spiritcrush = reinterpret_cast<interrogation *>(cr->activity.arg)->totalspiritcrush / cr->joindays;
        else
            spiritcrush = 0;
        ...
        while(1) {
            ...
            if(!techniques[TECHNIQUE_KILL])
                set_color(COLOR_WHITE, COLOR_BLACK, techniques[TECHNIQUE_NOSLEEP]);

            move(y, 0);
            y += 1;
            addstr("D - ");

            if(!techniques[TECHNIQUE_NOSLEEP])
                addstr("No ");

            addstr("Sleep Deprivation   ");

            if(!techniques[TECHNIQUE_KILL])
                set_color(COLOR_WHITE, COLOR_BLACK, !techniques[TECHNIQUE_GIVEFOOD]);

            move(y, 0);
            y += 1;
            addstr("E - ");

            if(techniques[TECHNIQUE_GIVEFOOD])
                addstr("Not ");

            addstr("Given Food    ");

            if(!techniques[TECHNIQUE_KILL])
                set_color(COLOR_WHITE, COLOR_BLACK, !techniques[TECHNIQUE_GIVEWATER]);

            move(y, 0);
            y += 1;
            addstr("F - ");

            if(techniques[TECHNIQUE_GIVEWATER])
                addstr("Not ");
        ...
        if(techniques[TECHNIQUE_NOSLEEP] && !turned && cr->alive) { // No sleep
            ...
            case 0:
                addstr("listen to protest music");
                break;

            case 1:
                addstr("listen to public radio");
                break;

            case 2:
                addstr("watch network news tapes");
                break;

            case 3:
                addstr("watch revealing documentaries");
                break;
            }
        ...
        if(techniques[TECHNIQUE_BEAT] && !turned && cr->alive) { // Beating
            ...
            if(a->attval(ATTRIBUTE_HEART) == 1 && techniques[TECHNIQUE_PROPS]) {
                ...
                case 2:
                    addstr(" holds the hostage's head under water");
                    break;

                case 3:
                    addstr(" peels back the Automaton's fingernails");
                    break;//XXX: but shouldn't this only happen once?

                case 4:
                    addstr(" beats the hostage with a metal bat");
                    break;

                case 5:
                    addstr(" beats the hostage with a belt");
                    break;
//            case 6:addstr(" ");break;
                }
                ...
                while(i < 2) {
                    ...
                    case 2:
                        addstr("Nobody loves you");
                        break;

                    case 3:
                        addstr("God hates you");
                        break;
                    ...
                    case 6:
                        addstr("Convert, bitch");
                        break;

                    case 7:
                        addstr("I'm going to kill you");
                        break;

                    case 8:
                        addstr("Do you love me?");
                        break;

                    case 9:
                        addstr("I am your God");
                        break;
                    }
                ...
                if(a->attval(ATTRIBUTE_HEART) > 1)
                    a->att[ATTRIBUTE_HEART]--;

                if(cr->attval(ATTRIBUTE_HEART) > 1)
                    cr->att[ATTRIBUTE_HEART]--;
            } else {
                if(temppool.size() == 1) {
                ...
                while(i < 3) {
                    ...
                    case 10:
                        addstr("War on Terror");
                        break;

                    case 11:
                        addstr("Ronald Reagan");
                        break;

                    case 12:
                        addstr("Rush Limbaugh");
                        break;
                    ...
                    case 16:
                        addstr("Deregulation");
                        break; //Cut a few that weren't names Conservatives use (or things they might like) -Fox
            ...
            if(forceroll >= cr->attval(ATTRIBUTE_HEALTH)) {
                if(cr->skillval(SKILL_RELIGION) > spiritcrush) {
                    ...
                    case 0:
                        ...
                        else if(cr->skillval(SKILL_RELIGION))
                            addstr("God's mercy.");
                        else
                            addstr("mommy.");
        ...
        if(techniques[TECHNIQUE_TALK] && cr->alive) {
            ...
            if(techniques[TECHNIQUE_PROPS]) { //props
                switch(LCSrandom(9)) {
                case 0:
                    addstr(" plays violent video games with ");
                    break;

                case 1:
                    addstr(" reads Origin of the Species to ");
                    break;

                case 2:
                    addstr(" burns flags in front of ");
                    break;

                case 3:
                    addstr(" explores an elaborate political fantasy with ");
                    break;

                case 4:
                    addstr(" watches controversial avant-garde films with ");
                    break;
                ...
                case 7:
                    addstr(" watches Michael Moore films with ");
                    break;

                case 8:
                    addstr(" listens to Liberal radio shows with ");
                    break;
                }
            } else {
                char str[75];
            ...
            //who has frequently abused this target in the past
                ...
                case 5:
                    addstr(" is too terrified to even speak to ");
                    addstr(a->name);
                    addstr(".");
                    break;
                ...
                if(a->attval(ATTRIBUTE_HEART) +
                    ...
                    case 6:
                        addstr("'s patience and kindness leaves the Conservative confused.");
                        break;
                    }
                    ...
                    if(rapport[a->id] > 3) {
                        ...
                        case 2:
                            addstr(" promises to be good.");
                            break;

                        case 3:
                            addstr(" reveals childhood pains.");
                            break;

                        case 4:
                            addstr(" thanks ");
                            addstr(a->name);
                            addstr(" for being merciful.");
                            break;
            ...
            //and rapport contribute to the likelihood of this
                ...
                if(cr->juice > 0) {
                    ...
                    if(cr->attval(ATTRIBUTE_WISDOM) > a->attval(ATTRIBUTE_WISDOM)) {
                        ...
                        if(change > a->skillval(SKILL_PSYCHOLOGY) / 2 + 1)
                            change = a->skillval(SKILL_PSYCHOLOGY) / 2 + 1;
                ...
                if(cr->attval(ATTRIBUTE_HEART) > cr->attval(ATTRIBUTE_WISDOM) + 4)
                    turned = 1;
                ...
                switch(LCSrandom(5)) {
                case 0:
                    addstr("'s Conservative beliefs are shaken.");
                    break;

                case 1:
                    addstr(" quietly considers these ideas.");
                    break;

                case 2:
                    addstr(" is beginning to see Liberal reason.");
                    break;

                case 3:
                    addstr(" has a revelation of understanding.");
                    break;

                case 4:
                    addstr(" grudgingly admits sympathy for LCS ideals.");
                    break;
                }
        ...
        if(cr->att[ATTRIBUTE_HEART] < 1)
            cr->att[ATTRIBUTE_HEART] = 1;

        //Lead interrogator gets bonus experience
        if(!techniques[TECHNIQUE_KILL])
            a->train(SKILL_PSYCHOLOGY, spiritcrush);
