...
void setpriority(newsstoryst &ns) {
    ...
    case NEWSSTORY_CARTHEFT: {
        ...
        if(crime[CRIME_STOLEGROUND] > 10)
            crime[CRIME_STOLEGROUND] = 10;

        if(crime[CRIME_BROKEDOWNDOOR] > 10)
            crime[CRIME_BROKEDOWNDOOR] = 10;

        if(crime[CRIME_ATTACKED_MISTAKE] > 10)
            crime[CRIME_ATTACKED_MISTAKE] = 10;

        if(crime[CRIME_ATTACKED] > 10)
            crime[CRIME_ATTACKED] = 10;

        if(crime[CRIME_BREAK_SWEATSHOP] > 10)
            crime[CRIME_BREAK_SWEATSHOP] = 10;

        if(crime[CRIME_BREAK_FACTORY] > 10)
            crime[CRIME_BREAK_FACTORY] = 10;

        if(crime[CRIME_FREE_RABBITS] > 10)
            crime[CRIME_BREAK_FACTORY] = 10;

        if(crime[CRIME_FREE_BEASTS] > 10)
            crime[CRIME_BREAK_FACTORY] = 10;
    ...
    case NEWSSTORY_KIDNAPREPORT:
        ...
        if(ns.cr->type == CREATURE_CORPORATE_CEO ||
                ...
                ns.cr->type == CREATURE_JUDGE_CONSERVATIVE)
            ns.priority = 40;
...
void displaystory(newsstoryst &ns, bool liberalguardian, int header) {
    ...
    if(!liberalguardian) {
        ...
        if(ns.guardianpage >= 3)
            adnumber++;

        if(ns.guardianpage >= 6)
            adnumber += LCSrandom(2) + 1;

        if(ns.guardianpage >= 9)
            adnumber += LCSrandom(2) + 1;

        if(ns.guardianpage >= 12)
            adnumber += LCSrandom(2) + 1;

        if(ns.guardianpage >= 15)
            adnumber += LCSrandom(2) + 1;
    }
    ...
    while(adnumber > 0) {
        ...
        for(y = sy; y <= ey; y++) {
            if(storyx_s[y] < ex && ex < 43)
                storyx_s[y] = ex + 2;

            if(storyx_e[y] > sx && sx > 43)
                storyx_e[y] = sx - 2;
        ...
        if(ns.positive) {
            ...
            */
            ...
            case VIEW_SWEATSHOPS:
                ...
                if(month >= 8 && month <= 11)
                    displaycenteredsmallnews("Fall fashions hit the stores across the country.", 12);
                else
                    displaycenteredsmallnews("Fall fashions are previewed in stores across the country.", 12);
        ...
        default: {
            ...
            if(liberalguardian) {
                ...
                case SITE_BUSINESS_CIGARBAR:
                    strcat(story, ", a spawning ground of Wrong Conservative Ideas.  ");
                    break;

                case SITE_LABORATORY_COSMETICS:
                    strcat(story, ", a Conservative animal rights abuser.  ");
                    break;

                case SITE_LABORATORY_GENETIC:
                    strcat(story, ", a dangerous Conservative genetic research lab.  ");
                    break;

                case SITE_GOVERNMENT_POLICESTATION:
                    strcat(story, ", headquarters of one of the most oppressive and Conservative police forces in the country.  ");
                    break;

                case SITE_GOVERNMENT_COURTHOUSE:
                    strcat(story, ", site of numerous Conservative Injustices.  ");
                    break;

                case SITE_GOVERNMENT_PRISON:
                    strcat(story, ", where innocent people are regularly beaten by Conservative guards.  ");
                    break;
                ...
                case SITE_INDUSTRY_SWEATSHOP:
                    strcat(story, ", a Conservative sweatshop and human rights abuser.  ");
                    break;

                case SITE_INDUSTRY_POLLUTER:
                    strcat(story, ", a factory whose Conservative smokestacks pollute choke the city with deadly pollutants.  ");
                    break;

                case SITE_INDUSTRY_NUCLEAR:
                    strcat(story, ", also known to be a Conservative storage facility for radioactive waste.  ");
                    break;

                case SITE_CORPORATE_HEADQUARTERS:
                    strcat(story, ", where evil and Conservatism coagulate in the hallways.  ");
                    break;

                case SITE_CORPORATE_HOUSE:
                    strcat(story, ", a building with enough square footage enough to house a hundred people if it weren't in Conservative Hands.  ");
                    break;
            ...
            if(ns.type == NEWSSTORY_SQUAD_KILLED_SITE) {
                if(liberalguardian)
                    strcat(story, "Everyone in the LCS group was tragically slain by the forces of evil.  ");
                else if(ns.positive)
                    strcat(story, "Everyone in the LCS group was killed by conservatives.  ");
                else
                    strcat(story, "Everyone in the LCS group was killed by brave citizens.  ");
            }
            ...
            for(int c = 0; c < ns.crime.size(); c++) {
                ...
                if(ns.crime[c] == CRIME_CORP_FILES)
                    continue;
                ...
                if(ns.crime[c] == CRIME_COURTHOUSE_LOCKUP)
                    continue;

                if(ns.crime[c] == CRIME_PRISON_RELEASE)
                    continue;

                if(ns.crime[c] == CRIME_JURYTAMPERING)
                    continue;
                ...
                if(ns.crime[c] == CRIME_CARCHASE)
                    continue;

                if(ns.crime[c] == CRIME_CARCRASH)
                    continue;

                if(ns.crime[c] == CRIME_FOOTCHASE)
                    continue;

                if(crime[ns.crime[c]] == 1)
                    typesum++;
            }
    ...
    case NEWSSTORY_MASSACRE: {
        ...
        if(!liberalguardian) {
            ...
            if(ns.crime[1] > 1)
                strcat(story, "victims were ");
            else
                strcat(story, "victim was ");
        ...
        case SIEGE_CIA:
            if(!liberalguardian) {
                if(ns.crime[1] > 1)
                    ...
                    strcat(story, "The body had no face or ");
    ...
    if(positive) {
        ...
        case VIEW_GAY: {
            ...
            if(law[LAW_GAY] <= -1)
                ...
                strcat(story, ", a homosexual, was ");

            switch(LCSrandom(3)) {
            case 0:
                strcat(story, "dragged to death behind a pickup truck");
                break;

            case 1:
                strcat(story, "burned alive");
                break;

            case 2:
                strcat(story, "beaten to death");
                break;
            }
            ...
            switch(LCSrandom(3)) {
            case 0:
                strcat(story, "throwing beer bottles");
                break;

            case 1:
                strcat(story, "urinating out the window");
                break;
            ...
            switch(LCSrandom(3)) {
            case 0:
                strcat(story, "the suspects ran out of gas, ");
                break;

            case 1:
                strcat(story, "the suspects collided with a manure truck, ");
                break;

            case 2:
                strcat(story, "the suspects veered into a ditch, ");
                break;
            }
            ...
            if(law[LAW_GAY] == -2)
                strcat(story, ", even though being gay is deviant, as we all know.");
            else
                strcat(story, ".");
        ...
        case VIEW_DEATHPENALTY: {
            ...
            switch(LCSrandom(3)) {
            case 0:
                strcat(story, "a confession from another convict.  ");
                break;

            case 1:
                strcat(story, "a battery of negative DNA tests.  ");
                break;
            ...
            switch(LCSrandom(3)) {
            case 0:
                strcat(story, "Let's not forget the convict is colored.  You know how their kind are");
                break;
            ...
            case 2:
                strcat(story, "The family wants closure.  We don't have time for another trial");
                break;
            }
        ...
        case VIEW_PRISONS: {
            ...
            case 3:
                strcat(story, "American");
                break;

            case 4:
                strcat(story, "Solitary");
                break;
            }
        ...
        case VIEW_INTELLIGENCE: {
            ...
            switch(LCSrandom(2)) {
            case 0:
                strcat(story, "buying music with 'Explicit Lyrics' labels.");
                break;

            case 1:
                strcat(story, "helping homeless people");
                break;
            }
        ...
        case VIEW_FREESPEECH: {
            ...
            case 3:
                strcat(story, "Invisible");
                break;

            case 4:
                strcat(story, "Wondrous");
                break;
            }
            ...
            case 3:
                strcat(story, "Whatever");
                break;

            case 4:
                strcat(story, "Something");
                break;
            }
            ...
            case 2:
                strcat(story, "Chinese");
                break;

            case 3:
                strcat(story, "Rwandan");
                break;

            case 4:
                strcat(story, "Palestinian");
                break;
            }
            ...
            switch(LCSrandom(5)) {
            case 0:
                strcat(story, "glorify Satan worship and are spawned by demons from the pit.  ");
                break;

            case 1:
                strcat(story, "teach children to kill their parents and hate life.  ");
                break;

            case 2:
                strcat(story, "cause violence in schools and are a gateway to cocaine use.  ");
                break;

            case 3:
                strcat(story, "breed demonic thoughts that manifest themselves as dreams of murder.  ");
                break;

            case 4:
                strcat(story, "contain step-by-step instructions to summon the Prince of Darkness.  ");
                break;
            }
            ...
            switch(LCSrandom(3)) {
            case 0:
                strcat(story, "a child that swore in class");
                break;

            case 1:
                strcat(story, "a child that said a magic spell at her parents");
                break;
        ...
        case VIEW_JUSTICES: {
            ...
            switch(LCSrandom(2)) {
            case 0:
                strcat(story, "defied the federal government by putting a Ten Commandments monument in the local federal building");
                break;

            case 1:
                strcat(story, "stated that, \"Segregation wasn't the bad idea everybody makes it out to be these days\"");
                break;
            }
            ...
            switch(LCSrandom(3)) {
            case 0:
                strcat(story, "\"the most perverse and spine-tingling debauchery imaginable, at least with only two people.\"");
                break;

            case 1:
                strcat(story, "the judge relieving himself on the prostitute.");
                break;

            case 2:
                strcat(story, "the prostitute hollering like a cowboy astride the judge.");
                break;
            }
            ...
            switch(LCSrandom(3)) {
            case 0:
                strcat(story, "the arresting officers money");
                break;

            case 1:
                strcat(story, "to let the officers join in");
                break;

            case 2:
                strcat(story, "the arresting officers \"favors\"");
                break;
            }
        ...
        case VIEW_AMRADIO: {
            ...
            switch(LCSrandom(3)) {
            case 0:
                strcat(story, "and the Grays are going to take over the planet in the End Times");
                break;

            case 1:
                strcat(story, "a liberal chupacabra will suck the blood from us like a goat, a goat!, a goat!");
                break;

            case 2:
                strcat(story, "I feel translucent rods passing through my body...  it's like making love to the future");
                break;
            }
            ...
            switch(LCSrandom(3)) {
            case 0:
                strcat(story, "my old hero");
                break;

            case 1:
                strcat(story, "my old idol");
                break;

            case 2:
                strcat(story, "the legend");
                break;
            }
            ...
            case 1:
                strcat(story, "maybe gone a little off the deep end");
                break;

            case 2:
                strcat(story, "probably been listening to Art Bell in the next studio a little too long");
                break;
            }
        ...
        case VIEW_DEATHPENALTY: {
            ...
            switch(LCSrandom(5)) {
            case 0:
                strcat(story, "pieces of another victim");
                break;

            case 1:
                strcat(story, "bloody toys");
                break;

            case 2:
                strcat(story, "a child's clothing stained with DNA evidence");
                break;

            case 3:
                strcat(story, "seven junior high school yearbooks");
                break;

            case 4:
                strcat(story, "two small backpacks");
                break;
            }
            ...
            switch(LCSrandom(5)) {
            case 0:
                strcat(story, "a victim called 911 just prior to being slain while still on the phone");
                break;

            case 1:
                strcat(story, "the suspect allegedly carved an address into one of the bodies");
                break;

            case 2:
                strcat(story, "an eye witness allegedly spotted the suspect luring a victim into a car");
                break;

            case 3:
                strcat(story, "a blood trail was found on a road that led them to the suspect's car trunk");
                break;

            case 4:
                strcat(story, "they found a victim in a ditch, still clinging to life");
                break;
            }
        ...
        case VIEW_ANIMALRESEARCH: {
            ...
            switch(LCSrandom(5)) {
            case 0:
                strcat(story, "boosts intelligence in chimpanzees");
                break;

            case 1:
                strcat(story, "corrects erectile dysfunction in chimpanzees");
                break;

            case 2:
                strcat(story, "allows chimpanzees to move blocks with their minds");
                break;

            case 3:
                strcat(story, "allows chimpanzees to fly short distances");
                break;

            case 4:
                strcat(story, "increases the attention span of young chimpanzees");
                break;
            }
            ...
            strcat(story,
                   "a spokesperson for the research team stated that, \"It really isn't so bad as all that.  Chimpanzees are very resilient creatures.  ");

            switch(LCSrandom(3)) {
            case 0:
                strcat(story, "The ones that survived are all doing very well");
                break;

            case 1:
                strcat(story, "They hardly notice when you drill their brains out, if you're fast");
                break;

            case 2:
                strcat(story, "When we started muffling the screams of our subjects, the other chimps all calmed down quite a bit");
                break;
            }
        ...
        case VIEW_PRISONS: {
            ...
            switch(LCSrandom(2)) {
            case 0:
                strcat(story, "himself");
                break;

            case 1:
                strcat(story, "herself");
                break;
            }
            ...
            switch(LCSrandom(3)) {
            case 0:
                strcat(story, "Ah, f*ck this sh*t.  This punk b*tch is f*ckin' dead!");
                break;

            case 1:
                strcat(story, "F*ck a m*th*f*ck*n' bull.  I'm killin' this pig sh*t.");
                break;
            ...
            case 1:
                strcat(story, "strangled the guard to death with a knotted bed sheet");
                break;
        ...
        case VIEW_INTELLIGENCE: {
            ...
            switch(LCSrandom(3)) {
            case 0:
                strcat(story, "white supremacists");
                break;

            case 1:
                strcat(story, "Islamic fundamentalists");
                break;

            case 2:
                strcat(story, "outcast goths from a suburban highschool");
                break;
            }
            ...
            switch(LCSrandom(9)) {
            case 0:
                strcat(story, "fly planes into skyscrapers");
                break;

            case 1:
                strcat(story, "detonate a fertilizer bomb at a federal building");
                break;

            case 2:
                strcat(story, "ram a motorboat loaded with explosives into a warship");
                break;

            case 3:
                strcat(story, "detonate explosives on a school bus");
                break;

            case 4:
                strcat(story, "blow out a section of a major bridge");
                break;

            case 5:
                strcat(story, "kidnap the president");
                break;

            case 6:
                strcat(story, "assassinate the president");
                break;

            case 7:
                strcat(story, "destroy the Capitol Building");
                break;

            case 8:
                strcat(story, "detonate a nuclear bomb in New York");
                break;
            }
        ...
        case VIEW_GENETICS: {
            ...
            switch(LCSrandom(5)) {
            case 0:
                strcat(story, "Altered");
                break;

            case 1:
                strcat(story, "Gene-tech");
                break;
            ...
            case 3:
                strcat(story, "Proteomic");
                break;

            case 4:
                strcat(story, "Genomic");
                break;
            }
            ...
            case 2:
                strcat(story, "Overlord");
                break;

            case 3:
                strcat(story, "Franken");
                break;

            case 4:
                strcat(story, "Transcendent");
                break;
            }
            ...
            switch(LCSrandom(5)) {
            case 0:
                strcat(story, "extends human life by a few minutes every bite");
                break;

            case 1:
                strcat(story, "mends split-ends upon digestion.  Hair is also made glossier and thicker");
                break;

            case 2:
                strcat(story, "allows people to see in complete darkness");
                break;

            case 3:
                strcat(story, "causes a person to slowly attain their optimum weight with repeated use");
                break;

            case 4:
                strcat(story, "cures the common cold");
                break;
            }
            ...
            switch(LCSrandom(4)) {
            case 0:
                strcat(story, "guy going on a killing spree");
                break;

            case 1:
                strcat(story, "gal turning blue and exploding");
                break;

            case 2:
                strcat(story, "guy speaking in tongues and worshipping Satan");
                break;

            case 3:
                strcat(story, "gal having a ruptured intestine");
                break;
            }
            ...
            case 1:
                strcat(story, "poppycock");
                break;

            case 2:
                strcat(story, "horse radish");
                break;

            case 3:
                strcat(story, "skunk weed");
                break;
            }
        ...
        case VIEW_JUSTICES: {
            ...
            switch(LCSrandom(5)) {
            case 0:
                strcat(story, "ten-year-old eye witness testimony");
                break;

            case 1:
                strcat(story, gen);
                strcat(story, " general feeling about police corruption");
                break;

            case 2:
                strcat(story, gen);
                strcat(story, " belief that the crimes were a right-wing conspiracy");
                break;
            ...
            case 4:
                strcat(story, gen);
                strcat(story, " personal philosophy of liberty");
                break;
            }
        ...
        case VIEW_POLLUTION:
            ...
            switch(LCSrandom(5)) {
            case 0:
                strcat(story, "Heritage");
                break;

            case 1:
                strcat(story, "Enterprise");
                break;

            case 2:
                strcat(story, "Freedom");
                break;

            case 3:
                strcat(story, "Liberty");
                break;

            case 4:
                strcat(story, "Charity");
                break;
            }
            ...
            switch(LCSrandom(5)) {
            case 0:
                strcat(story, "Partnership");
                break;

            case 1:
                strcat(story, "Institute");
                break;

            case 2:
                strcat(story, "Consortium");
                break;
            ...
            switch(LCSrandom(5)) {
            case 0:
                strcat(story, "a modest intake of radiactive waste");
                break;

            case 1:
                strcat(story, "a healthy dose of radiation");
                break;

            case 2:
                strcat(story, "a bath in raw sewage");
                break;

            case 3:
                strcat(story, "watching animals die in oil slicks");
                break;

            case 4:
                strcat(story, "inhaling carbon monoxide");
                break;
            }
            ...
            switch(LCSrandom(5)) {
            case 0:
                strcat(story, "purify the soul");
                break;

            case 1:
                strcat(story, "increase test scores");
                break;

            case 2:
                strcat(story, "increase a child's attention span");
                break;

            case 3:
                strcat(story, "make children behave better");
                break;

            case 4:
                strcat(story, "make shy children fit in");
                break;
            }
            ...
            switch(LCSrandom(3)) {
            case 0:
                strcat(story, "Research is complicated, and there are always two ways to think about things");
                break;

            case 1:
                strcat(story, "The jury is still out on pollution.  You really have to keep an open mind");
                break;

            case 2:
                strcat(story, "They've got their scientists, and we have ours.  The issue of pollution is wide open as it stands today");
                break;
            }
            ...
            switch(LCSrandom(3)) {
            case 0:
                strcat(story, "the elitist liberal media often distorts");
                break;

            case 1:
                strcat(story, "the vast left-wing education machine often distorts");
                break;

            case 2:
                strcat(story, "the fruits, nuts, and flakes of the environmentalist left often distort");
                break;
            }
        ...
        case VIEW_AMRADIO: {
            ...
            switch(LCSrandom(5)) {
            case 0:
                strcat(story, "Morning");
                break;

            case 1:
                strcat(story, "Commuter");
                break;
            ...
            switch(LCSrandom(5)) {
            case 0:
                strcat(story, "had intercourse");
                break;

            case 1:
                strcat(story, "encouraged listeners to call in and relieve themselves");
                break;

            case 2:
                strcat(story,
                       "screamed \"f*ck the police those g*odd*mn m*th*f*ck*rs.  I got a f*cking ticket this morning and I'm f*cking p*ss*d as sh*t.\"");
                break;

            case 3:
                strcat(story, "breastfed from a lactating woman");
                break;

            case 4:
                strcat(story, "masturbated");
                break;
            }
...
void majornewspaper(char &clearformess, char canseethings) {
    ...
    for(i = 0; i < pool.size(); i++) {
        if(pool[i]->alive && pool[i]->activity.type == ACTIVITY_WRITE) {
    ...
    if(!LCSrandom(VIEWNUM * 5)) {
        ...
        do {
            ...
            //NO ABORTION
            if(ns->view == VIEW_ABORTION && ns->positive && law[LAW_ABORTION] == -2)
                continue;
            ...
            //NO DEATH PENALTY
            if(ns->view == VIEW_DEATHPENALTY && law[LAW_ABORTION] == 2)
                continue;

            //NO NUCLEAR POWER
            if(ns->view == VIEW_NUCLEARPOWER && ns->positive && law[LAW_NUCLEARPOWER] == 2)
                continue;

            //NO ANIMAL RESEARCH
            if(ns->view == VIEW_ANIMALRESEARCH && law[LAW_ANIMALRESEARCH] == 2)
                continue;

            //NO BAD COPS
            if(ns->view == VIEW_POLICEBEHAVIOR && ns->positive && law[LAW_POLICEBEHAVIOR] == 2)
                continue;

            //NO PRIVACY VIOLATIONS
            if(ns->view == VIEW_INTELLIGENCE && ns->positive && law[LAW_PRIVACY] == 2)
                continue;

            //NO SWEATSHOPS
            if(ns->view == VIEW_SWEATSHOPS && ns->positive && law[LAW_LABOR] == 2)
                continue;

            //NO POLLUTION
            if(ns->view == VIEW_POLLUTION && ns->positive && law[LAW_POLLUTION] >= 1)
                continue;

            //NO ENRONS
            if(ns->view == VIEW_CORPORATECULTURE && ns->positive && law[LAW_CORPORATE] == 2)
                continue;

            //NO CEOS
            if(ns->view == VIEW_CEOSALARY && ns->positive && law[LAW_CORPORATE] == 2)
                continue;

            //NO FREEDOM OF SPEECH
            if(ns->view == VIEW_AMRADIO && !ns->positive && law[LAW_FREESPEECH] == -2)
                continue;
        ...
        if(ns->positive)
            change_public_opinion(ns->view, 20, 0);
        else
            change_public_opinion(ns->view, -20, 0);
    }
            ...
            if(newsstory[n]->type == NEWSSTORY_MAJOREVENT) {
                if(newsstory[n]->positive) {
                    ...
                    case VIEW_CABLENEWS: {
                        ...
                        case 2:
                            strcat(str, "Lightning");
                            break;

                        case 3:
                            strcat(str, "Washington");
                            break;
    ...
    if(newsstory.size()) {
        ...
        do {
            ...
            if(maxn != -1) {
                if(newsstory[maxn]->priority < 30 && curpage == 1)
                    curpage = 2;

                if(newsstory[maxn]->priority < 25 && curpage < 3)
                    curpage = 3 + LCSrandom(2);

                if(newsstory[maxn]->priority < 20 && curpage < 5)
                    curpage = 5 + LCSrandom(5);

                if(newsstory[maxn]->priority < 15 && curpage < 10)
                    curpage = 10 + LCSrandom(10);

                if(newsstory[maxn]->priority < 10 && curpage < 20)
                    curpage = 20 + LCSrandom(10);

                if(newsstory[maxn]->priority < 5 && curpage < 30)
    ...
    for(n = 0; n < newsstory.size(); n++) {
        if(newsstory[n]->type == NEWSSTORY_SQUAD_SITE ||
            ...
            if(newsstory[n]->page == 1)
                power *= 5;
            else if(newsstory[n]->page == 2)
                power *= 3;
            else if(newsstory[n]->page == 3)
                power *= 2;
            ...
            if(newsstory[n]->page == 1)
                maxpower = 100;
            else if(newsstory[n]->page < 5)
                maxpower = 100 - 10 * newsstory[n]->page;
            else if(newsstory[n]->page < 10)
                maxpower = 40;
            else if(newsstory[n]->page < 20)
                maxpower = 20;
            else if(newsstory[n]->page < 30)
                maxpower = 10;
            else if(newsstory[n]->page < 40)
                maxpower = 5;
            else
                maxpower = 1;

            if(newsstory[n]->positive == 2)
                power *= 3;
            ...
            if(newsstory[n]->positive)
                change_public_opinion(VIEW_LIBERALCRIMESQUADPOS, power, 0);
            else
                change_public_opinion(VIEW_LIBERALCRIMESQUADPOS, -power, 0);
            ...
            case SITE_GOVERNMENT_COURTHOUSE:
                ...
                change_public_opinion(VIEW_JUSTICES, power / 4 + 1, colored, power);
                change_public_opinion(VIEW_FREESPEECH, power / 2 + 1, colored, power);
