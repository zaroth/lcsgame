            ...
            if((c == 'p' || c == 'r' || c == 'w' || c == 'a' || c == 'l' || c == 'f') &&
                ...
                if(c == 'f')
                    ...
                    for(l = activesquad->loot.size() - 1; l >= 0; l--) {
                        ...
                        case ITEM_WEAPON:
                            fenceweapon[activesquad->loot[l]->weapon.type] += activesquad->loot[l]->number;
                            break;

                        case ITEM_ARMOR:
                            if(activesquad->loot[l]->armor.quality != '1')
                                break;

                            if(activesquad->loot[l]->armor.flag != 0)
                                break;
                        ...
                        case ITEM_CLIP:
                            fenceclip[activesquad->loot[l]->cliptype] += activesquad->loot[l]->number;
                            break;

                        case ITEM_LOOT:
                            fenceloot[activesquad->loot[l]->loottype] += activesquad->loot[l]->number;
                            break;
                        }
                    }
                    ...
                    for(l = activesquad->loot.size() - 1; l >= 0; l--) {
                        ...
                        case ITEM_WEAPON:
                            if(fenceweapon[activesquad->loot[l]->weapon.type] > 0) {
                                ...
                                if(numbersold > activesquad->loot[l]->number)
                                    numbersold = activesquad->loot[l]->number;
...
char maskselect(creaturest *cr, int16 &mask) {
    ...
    do {
        ...
        //PAGE DOWN
        if(c == interface_pgdn && (page + 1) * 19 < masktype.size())
            page++;
...
uint32 fenceselect(void) {
    ...
    do {
            ...
            if(slot >= 0 && slot < activesquad->loot.size()) {
                if(selected[slot]) {
                    ...
                    if(activesquad->loot[slot]->type == ITEM_ARMOR) {
                        if(activesquad->loot[slot]->armor.quality != '1')
                            bad = 1;

                        if(activesquad->loot[slot]->armor.flag != 0)
                            bad = 1;
                    }
                    ...
                    if(bad) {
                        ...
                        if(activesquad->loot[slot]->number > 1) {
                            ...
                            if(selected[slot] < 0)
                                selected[slot] = 0;
                            else if(selected[slot] > activesquad->loot[slot]->number)
                                selected[slot] = activesquad->loot[slot]->number;
                        } else
                            selected[slot] = 1;
        ...
        //PAGE DOWN
        if(c == interface_pgdn && (page + 1) * 18 < activesquad->loot.size())
            page++;
