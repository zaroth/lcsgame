...
void advanceday(char &clearformess, char canseethings) {
    ...
    for(p = 0; p < location.size(); ++p) {
        ...
        else if(location[p]->type == SITE_HOSPITAL_UNIVERSITY)
            healing[p] = 8;
        else
            healing[p] = 0;
    ...
    for(p = 0; p < pool.size(); p++) {
        ...
        if(clinictime(*pool[p])) {
            ...
            if(pool[p]->clinic)
                pool[p]->clinic = clinictime(*pool[p]);
            ...
            if((pool[p]->blood <= 20 || transfer) &&
                ...
                for(hospital = 0; hospital < location.size(); ++hospital) {
                    if(location[hospital]->type == SITE_HOSPITAL_UNIVERSITY)
                        break;
                }
    ...
    if(pool.size() > 0) {
            ...
            for(p = 0; p < pool.size(); p++) {
                ...
                if(pool[p]->hireid == -1) {
                    if(!disbanding) {
                        ...
                        dispersal_status[p] = DISPERSAL_BOSSINHIDING;
                }
                ...
                if(!pool[p]->alive && !disbanding) {
                    ...
                    if(promotesubordinates(*pool[p], clearformess))
                        promotion = 1;
        ...
        do { // while(changed!=0)
            ...
            for(p = pool.size() - 1; p >= 0; p--) {
                ...
                if(pool[p]->location != -1 &&
                        location[pool[p]->location]->type == SITE_GOVERNMENT_PRISON &&               !(pool[p]->flag & CREATUREFLAG_SLEEPER))
                    inprison = 1;
                else
                    inprison = 0;
...
bool promotesubordinates(Creature &cr, char &clearformess) {
    ...
    for(p = 0; p < pool.size(); p++) {
        ...
        if(pool[p]->hireid == cr.id && pool[p]->alive && pool[p]->align == 1) {
            ...
            if(bigboss == -1 && pool[p]->flag & CREATUREFLAG_BRAINWASHED)
                continue;
    ...
    if(bigboss == -2 || (cr.hireid != -1 && bigboss != -1 && !pool[bigboss]->alive))
        return 0;
...
void initlocation(locationst &loc) {
    ...
    case SITE_GOVERNMENT_PRISON:
        if(law[LAW_DEATHPENALTY] == -2 &&
            ...
            case 1:
                strcpy(loc.name, "Cheery");
                break;

            case 2:
                strcpy(loc.name, "Quiet");
                break;

            case 3:
                strcpy(loc.name, "Green");
                break;

            case 4:
                strcpy(loc.name, "Nectar");
                break;
            }
            ...
            switch(LCSrandom(5)) {
            case 0:
                strcat(loc.name, "Valley");
                break;

            case 1:
                strcat(loc.name, "Meadow");
                break;

            case 2:
                strcat(loc.name, "Hills");
                break;

            case 3:
                strcat(loc.name, "Glade");
                break;

            case 4:
                strcat(loc.name, "Forest");
                break;
            }
    ...
    case SITE_BUSINESS_JUICEBAR:
        ...
        switch(LCSrandom(5)) {
        case 0:
            strcat(loc.name, "Natural");
            break;

        case 1:
            strcat(loc.name, "Harmonious");
            break;

        case 2:
            strcat(loc.name, "Restful");
            break;

        case 3:
            strcat(loc.name, "Healthy");
            break;

        case 4:
            strcat(loc.name, "New You");
            break;
        }
        ...
        switch(LCSrandom(5)) {
        case 0:
            strcat(loc.name, "Diet");
            break;

        case 1:
            strcat(loc.name, "Methods");
            break;

        case 2:
            strcat(loc.name, "Plan");
            break;

        case 3:
            strcat(loc.name, "Orange");
            break;

        case 4:
            strcat(loc.name, "Carrot");
            break;
        }
    ...
    case SITE_BUSINESS_VEGANCOOP:
        ...
        switch(LCSrandom(5)) {
        case 0:
            strcat(loc.name, "Asparagus");
            break;

        case 1:
            strcat(loc.name, "Tofu");
            break;

        case 2:
            strcat(loc.name, "Broccoli");
            break;

        case 3:
            strcat(loc.name, "Radish");
            break;

        case 4:
            strcat(loc.name, "Eggplant");
            break;
        }
        ...
        case 1:
            strcat(loc.name, "Rainbow");
            break;

        case 2:
            strcat(loc.name, "Garden");
            break;

        case 3:
            strcat(loc.name, "Farm");
            break;
    ...
    case SITE_BUSINESS_INTERNETCAFE:
        ...
        switch(LCSrandom(5)) {
        case 0:
            strcat(loc.name, "Electric");
            break;

        case 1:
            strcat(loc.name, "Wired");
            break;

        case 2:
            strcat(loc.name, "Nano");
            break;

        case 3:
            strcat(loc.name, "Micro");
            break;

        case 4:
            strcat(loc.name, "Techno");
            break;
        }
        ...
        switch(LCSrandom(5)) {
        case 0:
            strcat(loc.name, "Panda");
            break;

        case 1:
            strcat(loc.name, "Troll");
            break;

        case 2:
            strcat(loc.name, "Latte");
            break;

        case 3:
            strcat(loc.name, "Unicorn");
            break;

        case 4:
            strcat(loc.name, "Pixie");
            break;
        }
    ...
    case SITE_BUSINESS_LATTESTAND:
        ...
        switch(LCSrandom(5)) {
        case 0:
            strcat(loc.name, "Frothy");
            break;

        case 1:
            strcat(loc.name, "Milky");
            break;

        case 2:
            strcat(loc.name, "Caffeine");
            break;

        case 3:
            strcat(loc.name, "Morning");
            break;

        case 4:
            strcat(loc.name, "Evening");
            break;
        }
        ...
        case 2:
            strcat(loc.name, "Jolt");
            break;

        case 3:
            strcat(loc.name, "Wonder");
            break;

        case 4:
            strcat(loc.name, "Express");
            break;
        }
