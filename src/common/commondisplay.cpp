...
void locheader(void) {
    ...
    if(activesquad != NULL) {
        ...
        if(location[selectedsiege]->siege.siege) {
            if(location[selectedsiege]->siege.underattack)
                set_color(COLOR_RED, COLOR_BLACK, 1);
            else
                set_color(COLOR_YELLOW, COLOR_BLACK, 1);
        } else
            ...
            if(party[p] != NULL) {
                ...
                for(int sk = 0; sk < SKILLNUM; sk++) {
                    ...
                    if(party[p]->get_skill_ip(sk) >= 100 + (10 * party[p]->skill[sk]) &&
                            party[p]->skill[sk] < maxskill(sk, *party[p]))
                        bright = 1;
                }
                ...
                if(showcarprefs == 1)
                    v = id_getcar(party[p]->pref_carid);
                else
                    v = id_getcar(party[p]->carid);
                ...
                if(v != -1 && showcarprefs != -1) {
                    ...
                    if((party[p]->wound[BODYPART_LEG_RIGHT] & WOUND_NASTYOFF) ||
                            (party[p]->wound[BODYPART_LEG_RIGHT] & WOUND_CLEANOFF))
                        legok--;

                    if((party[p]->wound[BODYPART_LEG_LEFT] & WOUND_NASTYOFF) ||
                            (party[p]->wound[BODYPART_LEG_LEFT] & WOUND_CLEANOFF))
                        legok--;

                    if(party[p]->flag & CREATUREFLAG_WHEELCHAIR)
...
void printlocation(long loc) {
    if(location[loc]->siege.siege) {
        if(!location[loc]->siege.underattack) {
            ...
            case SIEGE_POLICE:
                addstr("The police are raiding this location!");
                break;

            case SIEGE_CIA:
                addstr("The CIA is raiding this location!");
                break;

            case SIEGE_HICKS:
                addstr("The masses are storming this location!");
                break;

            case SIEGE_CORPORATE:
                addstr("The Corporations are raiding this location!");
                break;

            case SIEGE_CCS:
                addstr("The CCS is raiding this location!");
                break;

            case SIEGE_FIREMEN:
                addstr("Firemen are raiding this location!");
                break;
            }
        }
    } else {
    ...
    if(location[loc]->type == SITE_INDUSTRY_WAREHOUSE) {
        ...
        if(location[loc]->compound_stores != 1)
            addstr("s");
...
void printcreatureinfo(Creature *cr, unsigned char knowledge) {
    ...
    if(cr->prisoner != NULL) {
        ...
        case CREATURE_RADIOPERSONALITY:
            addstr("a crying Radio Personality");
            break;
        ...
        case CREATURE_SCIENTIST_EMINENT:
            addstr("a frightened Eminent Scientist");
            break;

        case CREATURE_JUDGE_CONSERVATIVE:
            addstr("an angry Hangin' Judge");
            break;
    ...
    while(snum > 0 && printed) {
        ...
        if(maxs != -1) {
            ...
            if(cr->get_skill_ip(maxs) >= 100 + (10 * cr->skill[maxs]) &&
                    cr->skill[maxs] < maxskill(maxs, *cr))
                set_color(COLOR_WHITE, COLOR_BLACK, 1);
            else
                set_color(COLOR_WHITE, COLOR_BLACK, 0);
...
void printliberalskills(Creature &cr) {
    ...
    for(int s = 0; s < SKILLNUM; s++) {
        ...
        // About to level up skills are white
                ...
                cr.skill[s] < maxskill(s, cr))
            set_color(COLOR_WHITE, COLOR_BLACK, 1);
        // <1 skills are dark gray
        else if(cr.skill[s] < 1)
            set_color(COLOR_BLACK, COLOR_BLACK, 1);
...
void printhealthstat(Creature &g, int y, int x, char smll) {
    ...
    if((g.wound[BODYPART_ARM_RIGHT] & WOUND_NASTYOFF) ||
            (g.wound[BODYPART_ARM_RIGHT] & WOUND_CLEANOFF))
        armok--;

    if((g.wound[BODYPART_ARM_LEFT] & WOUND_NASTYOFF) ||
            (g.wound[BODYPART_ARM_LEFT] & WOUND_CLEANOFF))
        armok--;

    if((g.wound[BODYPART_LEG_RIGHT] & WOUND_NASTYOFF) ||
            (g.wound[BODYPART_LEG_RIGHT] & WOUND_CLEANOFF))
        legok--;

    if((g.wound[BODYPART_LEG_LEFT] & WOUND_NASTYOFF) ||
            (g.wound[BODYPART_LEG_LEFT] & WOUND_CLEANOFF))
        legok--;
