...
int clinictime(Creature &g) {
    ...
    if(!g.special[SPECIALWOUND_RIGHTLUNG])
        time++;

    if(!g.special[SPECIALWOUND_LEFTLUNG])
        time++;

    if(!g.special[SPECIALWOUND_HEART])
        time += 2;

    if(!g.special[SPECIALWOUND_LIVER])
        time++;

    if(!g.special[SPECIALWOUND_STOMACH])
        time++;

    if(!g.special[SPECIALWOUND_RIGHTKIDNEY])
        time++;

    if(!g.special[SPECIALWOUND_LEFTKIDNEY])
        time++;

    if(!g.special[SPECIALWOUND_SPLEEN])
        time++;

    if(g.special[SPECIALWOUND_RIBS] < RIBNUM)
        time++;

    if(!g.special[SPECIALWOUND_NECK])
        time++;

    if(!g.special[SPECIALWOUND_UPPERSPINE])
        time++;

    if(!g.special[SPECIALWOUND_LOWERSPINE])
        time++;
    ...
    if(cr.squadid != -1) {
        ...
        if(sq != -1) {
            ...
            for(int pt = 0; pt < 6; pt++) {
                ...
                if(flipstart && pt < 5)
                    squad[sq]->squad[pt] = squad[sq]->squad[pt + 1];
            }

            if(flipstart)
                squad[sq]->squad[5] = NULL;
        }
...
int lawflagheat(int lawflag) {
    ...
    case LAWFLAG_ARMEDASSAULT:
        return 0;    // Fox: Yes. You get too many assault charges to put heat on it.
...
void sorting_prompt(short listforsorting) {
    ...
    case SORTINGCHOICE_HOSTAGES:
        addstr("hostages.");
        break;

    case SORTINGCHOICE_CLINIC:
        addstr("Liberals in hospital.");
        break;

    case SORTINGCHOICE_JUSTICE:
        addstr("oppressed Liberals.");
        break;

    case SORTINGCHOICE_SLEEPERS:
        addstr("sleepers.");
        break;

    case SORTINGCHOICE_DEAD:
        addstr("dead people.");
        break;

    case SORTINGCHOICE_AWAY:
        addstr("people away.");
        break;

    case SORTINGCHOICE_ACTIVATE:
        addstr("Liberal activity.");
        break;

    case SORTINGCHOICE_ACTIVATESLEEPERS:
        addstr("sleeper activity.");
        break;

    case SORTINGCHOICE_ASSEMBLESQUAD:
...
short reviewmodeenum_to_sortingchoiceenum(short reviewmode) {
    ...
    case REVIEWMODE_HOSTAGES:
        return SORTINGCHOICE_HOSTAGES;

    case REVIEWMODE_CLINIC:
        return SORTINGCHOICE_CLINIC;

    case REVIEWMODE_JUSTICE:
        return SORTINGCHOICE_JUSTICE;

    case REVIEWMODE_SLEEPERS:
        return SORTINGCHOICE_SLEEPERS;

    case REVIEWMODE_DEAD:
        return SORTINGCHOICE_DEAD;

    case REVIEWMODE_AWAY:
        return SORTINGCHOICE_AWAY;
    }
