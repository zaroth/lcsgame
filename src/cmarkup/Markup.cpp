...
int TextEncoding::IConv( void *pTo, int nToCharSize, int nFromCharSize ) {
    ...
    if ( cd != (iconv_t) - 1 ) {
        ...
        while ( nFromLenRemaining ) {
            ...
            nResult = iconv( cd, const_cast<const char **>(&pFromChar), &nFromLenRemaining, &pToChar,
                             &nToCountRemaining ); //Added const_cast<char**>() to second argument. It wouldn't compile otherwise. -XML
...
struct TokenPos {
    ...
    void ForwardUntil( MCD_PCSZ szStopChars ) {
        while ( m_pDocText[m_nNext] && ! MCD_PSZCHR(szStopChars, m_pDocText[m_nNext]) )
...
struct PathPos {
    ...
    void IncWord() {
        while (p[i] && !MCD_PSZCHR(MCD_T(" =/[]"), p[i]))
            i += MCD_CLEN(&p[i]);
    };
...
struct SavedPosMap {
    ...
    ~SavedPosMap() {
        if (pTable) {
            for (int n = 0; n < nMapSize; ++n) if (pTable[n])
                    delete[] pTable[n];
...
bool CMarkup::WriteTextFile( MCD_CSTR_FILENAME szFileName, const MCD_STR &strDoc, MCD_STR *pstrResult, int *pnDocFlags,
                             MCD_STR *pstrEncoding ) {
