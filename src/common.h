...
template <class Container> void delete_and_remove(Container &c, int i) {
    if(i <= -1)
        return;  // check for invalid index, and don't delete anything in this case

    if(c[i] != NULL)
        delete c[i];  // be able to hande null pointers just in case that happens
...
template <class Container> void delete_and_remove(Container &c1, int i1, Container &c2, int i2) {
    ...
    if(c1[i1] == c2[i2]) {
        ...
        if(c1 != c2 || i1 != i2)
            c2.erase(c2.begin() + i2);  // remove it from 2nd location, unless 2nd location is identical to 1st location
    } else {
...
template <class Container> void delete_and_clear(Container &c) {
    while(!c.empty()) {
        if(c.back() != NULL)
            delete c.back();  // be able to hande null pointers just in case that happens
