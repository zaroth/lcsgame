...
void elections(char clearformess, char canseethings) {
    ...
    if(year % 4 == 0) {
        ...
        for(int l = 0; l < 1000; l++) { // 1000 Voters!
            ...
            if((canseethings) && l % 5 == 4) {
                for(int c = 0; c < 2; c++) {
                    ...
                    else if(votes[c] > votes[!c] || c == winner)
                        set_color(COLOR_WHITE, COLOR_BLACK, 1);
                    else
                        set_color(COLOR_WHITE, COLOR_BLACK, 0);
        ...
        if(oldwinner)
            ...
            for(int e = 0; e < EXECNUM; e++) {
                ...
                if(candidate[winner][0] == -2)
                    exec[e] = -2;
                else if(candidate[winner][0] == 2)
                    exec[e] = 2;
                else
                    exec[e] = candidate[winner][0] + LCSrandom(3) - 1;
    ...
    for(p = 0; p < pnum; p++) {
        ...
        if(canseethings) {
            ...
            switch(p) {
            case 0:
                propnum = 2 * (17 - LCSrandom(2) * 6) * (19 - LCSrandom(2) * 6);
                break;

            case 1:
                propnum = 7 * (17 - LCSrandom(2) * 6) * (19 - LCSrandom(2) * 6);
                break;

            case 2:
                propnum = 3 * (17 - LCSrandom(2) * 6) * (19 - LCSrandom(2) * 6);
                break;

            case 3:
                propnum = 5 * (17 - LCSrandom(2) * 6) * (2 - LCSrandom(2) * 1);
                break;

            case 4:
                propnum = 11 * (17 - LCSrandom(2) * 6) * (2 - LCSrandom(2) * 1);
                break;

            case 5:
                propnum = 13 * (17 - LCSrandom(2) * 6) * (2 - LCSrandom(2) * 1);
                break;
            }
            ...
            case LAW_ABORTION:
                if(propdir[p] == 1)
                    addstr("Strengthen Abortion Rights");
                else
                    addstr("Protect the Unborn Child");

                break;

            case LAW_ANIMALRESEARCH:
                if(propdir[p] == 1)
                    addstr("Limit Animal Cruelty");
                else
                    addstr("Expand Animal Research");

                break;

            case LAW_POLICEBEHAVIOR:
                if(propdir[p] == 1)
                    addstr("Curtail Police Misconduct");
                else
                    addstr("Stop Harassment of Police Officers");
            ...
            case LAW_NUCLEARPOWER:
                if(propdir[p] == 1)
                    addstr("Limit Nuclear Power");
                else
                    addstr("Expand Nuclear Power");

                break;

            case LAW_POLLUTION:
                if(propdir[p] == 1)
                    addstr("Punish Polluters");
                else
                    addstr("Reward Industry");

                break;

            case LAW_LABOR:
                if(propdir[p] == 1)
                    addstr("Enhance Labor Standards");
                else
                    addstr("End Undue Union Influence");

                break;

            case LAW_GAY:
                if(propdir[p] == 1)
                    addstr("Expand Homosexual Rights");
                else
                    addstr("Support the Sanctity of Marriage");

                break;

            case LAW_CORPORATE:
                if(propdir[p] == 1)
                    addstr("Stop Corporate Criminals");
                else
                    addstr("Reward Small Businesses");

                break;

            case LAW_FREESPEECH:
                if(propdir[p] == 1)
                    addstr("Protect Free Speech");
                else
                    addstr("Limit Hurtful Speech");

                break;

            case LAW_TAX:
                if(propdir[p] == 1)
                    addstr("Punish the Wealthy");
                else
                    addstr("Stimulate Economic Growth");

                break;

            case LAW_FLAGBURNING:
                if(propdir[p] == 1)
                    addstr("Limit Prohibitions on Flag Burning");
                else
                    addstr("Protect the Symbol of Our Nation");

                break;

            case LAW_GUNCONTROL:
                if(propdir[p] == 1)
                    addstr("Prevent Gun Violence");
                else
                    addstr("Assert our Second Amendment Rights");

                break;

            case LAW_WOMEN:
                if(propdir[p] == 1)
                    addstr("Expand Women's Rights");
                else
                    addstr("Preserve Traditional Gender Roles");

                break;

            case LAW_CIVILRIGHTS:
                if(propdir[p] == 1)
                    addstr("Expand Civil Rights");
                else
                    addstr("Fight Reverse Discrimination");

                break;

            case LAW_DRUGS:
                if(propdir[p] == 1)
                    addstr("Limit Oppressive Drug Laws");
                else
                    addstr("Strengthen the War On Drugs");

                break;

            case LAW_IMMIGRATION:
                if(propdir[p] == 1)
                    addstr("Protect Immigrant Rights");
                else
                    addstr("Protect our Borders");

                break;

            case LAW_ELECTIONS:
                if(propdir[p] == 1)
                    addstr("Fight Political Corruption");
                else
                    addstr("Limit Campaign Finance Reforms");

                break;

            case LAW_MILITARY:
                if(propdir[p] == 1)
                    addstr("Limit Military Spending");
                else
                    addstr("Strengthen our National Defense");
...
void elections_senate(int senmod, char canseethings) {
    ...
    for(s = 0; s < 100; s++) {
        ...
        if(termlimits) {
            ...
            if(senate[s] > 0 && vote < 3 && LCSrandom(mood + 11) > 10)
                vote++;
            ...
            case -1:
                if(vote == 0 && LCSrandom(100 - mood + 1) > 60)
                    senate[s] = -2;

                if(mood < 50 && LCSrandom(7))
                    break;

                if(vote >= 3 && LCSrandom(3))
                    senate[s] = vote - 1;
            ...
            case 1:
                if(vote == 4 && LCSrandom(mood + 1) > 60)
                    senate[s] = 2;

                if(mood > 50 && LCSrandom(7))
                    break;

                if(vote <= 1 && LCSrandom(3))
                    senate[s] = vote - 1;
...
void elections_house(char canseethings) {
    ...
    for(h = 0; h < 435; h++) {
        ...
        if(termlimits) {
            ...
            if(house[h] > 0 && vote < 3 && LCSrandom(mood + 11) > 10)
                vote++;
...
void supremecourt(char clearformess, char canseethings) {
    ...
    for(c = 0; c < cnum; c++) {
        ...
        if(scase[c] == LAW_FREESPEECH || scase[c] == LAW_FLAGBURNING)
            bias = 1;
        else if(scase[c] == LAW_GUNCONTROL)
            bias = -1;
        else
            bias = 0;
        ...
        if(canseethings) {
            ...
            case LAW_GUNCONTROL:
                if(scasedir[c] == 1)
                    ...
                    addstr("Protect our Second Amendment Rights");
            ...
            case LAW_ELECTIONS:
                if(scasedir[c] == 1)
                    ...
                    addstr("Limit Regulation of Political Speech");
    ...
    if(LCSrandom(10) >= 6) {
        ...
        if(consensus < -1.5f)
            court[j] = -2;
        else if(consensus < -.5f)
            court[j] = -1;
        else if(consensus < .5f)
            court[j] = 0;
        else if(consensus < 1.5f)
            court[j] = 1;
        else
            court[j] = 2;
        ...
        if(canseethings) {
            ...
            case LAW_PRIVACY:
                if(billdir[c] == 1)
                    ...
                    addstr("Allow Corporations Information Access");
