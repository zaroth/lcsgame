...
void sleeper_spy(Creature &cr, char &clearformess, char canseethings, int *libpower) {
    ...
    case CREATURE_AGENT:
        ...
        if(!location[homes]->siege.siege && canseethings) {
            if(LCSrandom(law[LAW_PRIVACY] + 3))
                break;
    ...
    case CREATURE_GANGUNIT:
        ...
        if(!location[homes]->siege.siege && canseethings) {
            if(LCSrandom(law[LAW_POLICEBEHAVIOR] + 3))
                break;
    ...
    case CREATURE_CORPORATE_CEO:
        ...
        if(!location[homes]->siege.siege && canseethings) {
            if(LCSrandom(law[LAW_CORPORATE] + 3) && cr.type != CREATURE_CORPORATE_CEO)
                break;
    ...
    case CREATURE_NEWSANCHOR:
        if(!location[homes]->siege.siege && canseethings) {
            ...
            if(LCSrandom(law[LAW_FREESPEECH] + 3))
                break;
    ...
    case CREATURE_SCIENTIST_EMINENT:
        if(!location[homes]->siege.siege && canseethings) {
            if(LCSrandom(law[LAW_ANIMALRESEARCH] + 3))
                break;
    ...
    while(number_of_items--) {
        ...
        case SITE_GOVERNMENT_POLICESTATION:
            ...
            if(!LCSrandom(3)) {
                ...
                if(!LCSrandom(4))
                    item->weapon.type = WEAPON_SMG_MP5;
                else if(!LCSrandom(3))
                    item->weapon.type = WEAPON_SEMIPISTOL_45;
                else if(!LCSrandom(2))
                    item->weapon.type = WEAPON_SHOTGUN_PUMP;
                else
                    item->weapon.type = WEAPON_SEMIRIFLE_AR15;
            } else if(!LCSrandom(2)) {
                item->type = ITEM_ARMOR;

                if(!LCSrandom(3))
                    item->armor.type = ARMOR_POLICEUNIFORM;
                else if(!LCSrandom(2))
                    item->armor.type = ARMOR_SWATARMOR;
                else
        ...
        case SITE_GOVERNMENT_INTELLIGENCEHQ:
            ...
            if(!LCSrandom(3)) {
                ...
                if(!LCSrandom(4))
                    ...
                    item->weapon.type = WEAPON_AUTORIFLE_M16;
                else if(!LCSrandom(2))
                    item->weapon.type = WEAPON_SHOTGUN_PUMP;
                else
                    item->weapon.type = WEAPON_CARBINE_M4;
            } else if(!LCSrandom(2)) {
                item->type = ITEM_ARMOR;
    ...
    if(subordinatesleft(cr)) {
        ...
        for(int e = 0; e < 18; e++) {
            ...
            if(encounter[e].worklocation == cr.worklocation || !LCSrandom(5)) {
                if(encounter[e].align != 1 && LCSrandom(5))
                    continue;
