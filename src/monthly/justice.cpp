...
void trial(creaturest &g) {
    ...
    if(defense != 2) {
        ...
        if(jury <= -29) {
            ...
            case 0:
                addstr(g.name);
                addstr("'s best friend from childhood is a juror.");
                break;
        ...
        if(scarefactor > 15)
            prosecution = LCSrandom(51) + 100;
        else if(scarefactor > 5)
            prosecution = LCSrandom(101) + 50;
        else if(scarefactor > 0)
            prosecution = LCSrandom(91) + 10;
        else
...
void penalize(creaturest &g, char lenient) {
    ...
    if((g.lawflag[LAWFLAG_MURDER]) || (g.lawflag[LAWFLAG_TREASON]) ||
        ...
        if(law[LAW_DEATHPENALTY] == -1)
            g.deathpenalty = LCSrandom(3);

        if(law[LAW_DEATHPENALTY] == 0)
            g.deathpenalty = LCSrandom(2);

        if(law[LAW_DEATHPENALTY] == 1)
            g.deathpenalty = !LCSrandom(5);
    ...
    //CALC TIME
    if(!g.deathpenalty) {
        if((g.lawflag[LAWFLAG_KIDNAPPING]) && g.sentence != -1)
            g.sentence += 36 + LCSrandom(18);

        if((g.lawflag[LAWFLAG_THEFT]) && g.sentence != -1)
            g.sentence += 3 + LCSrandom(8);

        if((g.lawflag[LAWFLAG_GUNCARRY]) && g.sentence != -1)
            g.sentence += 12 + LCSrandom(48);

        if((g.lawflag[LAWFLAG_CARTHEFT]) && g.sentence != -1)
            g.sentence += 6 + LCSrandom(7);

        if((g.lawflag[LAWFLAG_INFORMATION]) && g.sentence != -1)
            g.sentence += 1 + LCSrandom(13);

        if((g.lawflag[LAWFLAG_COMMERCE]) && g.sentence != -1)
            g.sentence += 1 + LCSrandom(13);

        if((g.lawflag[LAWFLAG_CCFRAUD]) && g.sentence != -1)
            g.sentence += 6 + LCSrandom(25);

        if((g.lawflag[LAWFLAG_BURIAL]) && g.sentence != -1)
            g.sentence += 3 + LCSrandom(12);

        if((g.lawflag[LAWFLAG_PROSTITUTION]) && g.sentence != -1)
            g.sentence += 1 + LCSrandom(6);

        if((g.lawflag[LAWFLAG_DISTURBANCE]) && g.sentence != -1)
            g.sentence += 1;

        if((g.lawflag[LAWFLAG_LOITERING]) && g.sentence != -1)
            g.sentence += 1;

        if((g.lawflag[LAWFLAG_HIREILLEGAL]) && g.sentence != -1)
            g.sentence += 1;

        if((g.lawflag[LAWFLAG_RACKETEERING]) && g.sentence != -1)
            g.sentence += 6 + LCSrandom(100);
        ...
        if((g.lawflag[LAWFLAG_BROWNIES]) && g.sentence != -1) {
            if(LCSrandom(3))
                ...
                if(LCSrandom(3))
                    g.sentence += 3 + LCSrandom(120);
                else
                    g.sentence += 3 + LCSrandom(360);
            }
        }

        if((g.lawflag[LAWFLAG_BREAKING]) && g.sentence != -1)
            g.sentence += 1;

        if((g.lawflag[LAWFLAG_TERRORISM]) && g.sentence != -1)
            g.sentence += 60 + LCSrandom(181);

        if((g.lawflag[LAWFLAG_JURY]) && g.sentence != -1)
            g.sentence += 30 + LCSrandom(61);

        if((g.lawflag[LAWFLAG_ESCAPED]) && g.sentence != -1)
            g.sentence = -1;

        if((g.lawflag[LAWFLAG_HELPESCAPE]) && g.sentence != -1)
            g.sentence += 30 + LCSrandom(61);

        if((g.lawflag[LAWFLAG_RESIST]) && g.sentence != -1)
            g.sentence += 1 + LCSrandom(1);

        if((g.lawflag[LAWFLAG_BURNFLAG]) && g.sentence != -1) {
            if(law[LAW_FLAGBURNING] == -2) {
                if(!LCSrandom(2))
                    g.sentence += 120 + LCSrandom(241);
                else
                    g.sentence = -1;
            } else if(law[LAW_FLAGBURNING] == -1)
                g.sentence += 36;
        ...
        if((g.lawflag[LAWFLAG_SPEECH]) && g.sentence != -1)
            g.sentence += 4 + LCSrandom(3);

        if((g.lawflag[LAWFLAG_VANDALISM]) && g.sentence != -1)
            g.sentence += 1;

        if((g.lawflag[LAWFLAG_ASSAULT]) && g.sentence != -1)
            g.sentence += 3 + LCSrandom(1);
        ...
        if((g.lawflag[LAWFLAG_TREASON]) && g.sentence != -1)
            g.sentence = -1;
        ...
        if(g.sentence == 0) {
            ...
            if(g.deathpenalty) {
                ...
                if(law[LAW_DEATHPENALTY] == -2) {
                    ...
                    case 1:
                        addstr("drawing and quartering");
                        break;

                    case 2:
                        addstr("disemboweling");
                        break;

                    case 3:
                        addstr("one thousand cuts");
                        break;

                    case 4:
                        addstr("feeding the lions");
                        break;
                    ...
                    case 8:
                        addstr("head-squishing");
                        break;

                    case 9:
                        addstr("piranha tank swimming exhibition");
                        break;

                    case 10:
                        addstr("forced sucking of Ronald Reagan's ass");
                        break;

                    case 11:
                        addstr("covering with peanut butter and letting rats eat");
                        break;

                    case 12:
                        addstr("burying up to the neck in a fire ant nest");
                        break;

                    case 13:
                        addstr("running truck over the head");
                        break;
                    ...
                    case 15:
                        addstr("chipper-shredder");
                        break;

                    case 16:
                        addstr("use in lab research");
                        break;

                    case 17:
                        addstr("blood draining");
                        break;

                    case 18:
                        addstr("chemical weapons test");
                        break;

                    case 19:
                        addstr("sale to a furniture maker");
                        break;

                    case 20:
                        addstr("sale to a CEO as a personal pleasure toy");
                        break;

                    case 21:
                        addstr("sale to foreign slave traders");
                        break;

                    case 22:
                        addstr("exposure to degenerate Bay 12 Curses games");
                        break;
                    }
