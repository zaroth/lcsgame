...
class Item {
  public:
    ...
    explicit Item(const char *inputXml);
    virtual ~Item() {}
    ...
    virtual bool is_same_type(const Item &i);
    ...
    virtual bool is_clip() const {
        return false;
    }
    virtual bool is_loot() const {
        return false;
    }
    virtual bool is_money() const {
        return false;
    }

    virtual const string &get_name() const = 0;
    virtual long get_fencevalue() const = 0;
    virtual bool is_good_for_sale() const {
        return true;    //Have one that gives an error message when suitable? -XML
    }

    void set_number(long number) {
        number_ = number;
    }
    void increase_number(long increase) {
        number_ += increase;
    }
    void decrease_number(long decrease) {
        number_ -= decrease;
    }
    ...
    const string &get_itemtypename() const {
        return itemtypename_;
    }
    long get_itemtypeid() const {
        return itemtypeid_;
    }

  protected:
    //string& itemtypename() { return itemtypename_; }
    const string &itemtypename() const {
        return itemtypename_;
    }
    //long& itemtypeid() { return itemtypeid_; }
    const long &itemtypeid() const {
        return itemtypeid_;
    }
