...
char chasesequence(void) {
    ...
    for(p = 0; p < 6; p++) {
        ...
        if(activesquad->squad[p]->carid != -1) {
            ...
            if(v != -1) {
                for(v2 = 0; v2 < (int)chaseseq.friendcar.size(); v2++)if(chaseseq.friendcar[v2]->id() == vehicle[v]->id())
                        break;

                if(v2 == (int)chaseseq.friendcar.size())
...
void evasiverun(void) {
    ...
    for(int p = 0; p < 6; p++) {
        ...
        if(activesquad->squad[p]->alive) {
            ...
            if(yourworst > yourspeed[p])
                yourworst = yourspeed[p];

            if(yourbest < yourspeed[p])
                yourbest = yourspeed[p];
        }
    }
...
void makechasers(long sitetype, long sitecrime) {
    ...
    for(int c = 0; c < carnum; c++) {
        Vehicle *v = new Vehicle(*vehicletype[getvehicletype(
                cartype)]); //If car type is unknown, due to change in xml file, the game will crash here. -XML
    ...
    for(int p = 0; p < 6; p++) {
        ...
        if(activesquad->squad[p]->carid == chaseseq.friendcar[v]->id()) {
            ...
            if(activesquad->squad[p]->blood <= 0) {
                ...
                switch(LCSrandom(3)) {
                case 0:
                    addstr(" slumps in ", gamelog);
                    addstr(activesquad->squad[p]->hisher(), gamelog);
                    addstr(" seat, out cold, and dies.", gamelog);
                    break;

                case 1:
                    addstr(" is crushed by the impact.", gamelog);
                    break;

                case 2:
                    addstr(" struggles free of the car, then collapses lifelessly.", gamelog);
                    break;
                }
                ...
                case 2:
                    addstr(" crawls free of the car, shivering with pain.", gamelog);
...
void crashenemycar(int v) {
    ...
    case 1:
        ...
        if(victimsum > 1)
            addstr("Everyone inside is peeled off against the pavement.", gamelog);
        else if(victimsum == 1)
            addstr("The person inside is squashed into a cube.", gamelog);
...
void chase_giveup(void) {
    ...
    if(mode != GAMEMODE_CHASECAR)
        addstr("You stop and are arrested.", gamelog);
    else
        addstr("You pull over and are arrested.", gamelog);
